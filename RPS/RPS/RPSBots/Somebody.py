# n = 5
#No lot of comment anymore
#no reverse + no draw
#credit dllu part of this AI come from him
import random
...
if input =="":
    ...
    for i in range(num_predictors):
        ...
        #elif input==beat[beat[predictors[i]]]:
            score1[i]-=3
        #meta 2
        if input==predictors[i]:
            score2[i]+=3
        ...
        #elif output==beat[beat[predictors[i]]]:
           score3[i]-=3
        #meta 4
        if output==predictors[i]:
            score4[i]+=3
    ...
    for i in range (num_meta):
        if metapredictors[i]==input:
            ...
            result="0"
        list_meta[i] = list_meta[i][front:5]+result #only 5 rounds before
    ...
    for i in range(6,18):
        predictors[i]=beat[beat[predictors[i-6]]]
