...
if not input:
   ...
   if flag[2]:
      ...
      for j in range(3):
          subs[3][j+3]=mdl(k2i[mRNA[i+k]]-j-1)
   i=min(5,len(DNA))
   k=-1
   while i>1 and k<0:
         i-=1
   ...
   flag[2]=flag[2] and (k>=0)
   flag[3]=flag[2]
   if flag[2]:
      for j in range(3):
          subs[3][j]=mdl(k2i[tRNA[i+k]]-j+1)
      for j in range(3):
          subs[2][j+3]=mdl(k2i[mRNA[i+k]]-j-1)
   flag[4]=flag[2] and flag[3]
   if flag[4]:
      for i in range(2):
          for j in range(3):
              hist[i][j]=0
      for i in range(2):
          hist[i][mdl(subs[2][3*i]-1)]+=1
          hist[i][mdl(subs[3][3*i]-1)]+=1
      for i in range(2):
          for j in range(3):
              eval[j]=hist[i][mdl(j-1)]-hist[i][mdl(j+1)]
          k=eval.index(max(eval))
