...
if not input:
   ...
   if flag[1]:
      for i in range(3):
          eval[i]=hist[2][mdl(i-1)]-hist[2][mdl(i+1)]
      j=eval.index(max(eval))
      for i in range(3):
          subs[1][i]=mdl(j-i)
   i=-1
   ...
   while i<0 and j>1:
         ...
         RNA=mRNA[-j:]
         i=mRNA.find(RNA,0,-1)
   flag[1]=flag[1] and (i>=0)
   if flag[1]:
      k=mdl(k2i[mRNA[i+j]]-1)
      for i in range(3):
          subs[1][i+3]=mdl(k-i)
   output=random.choice("RPS")
