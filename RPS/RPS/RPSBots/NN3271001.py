...
def action():
  ...
  g = max(0.0, 5.0 * pred[1] - pred[2])
  c = g + max(0.0, 5.0 * pred[2] - pred[0])
  p = c + max(0.0, 5.0 * pred[0] - pred[1])
  if p == 0.0:
...
def result(mine, others):
  scale = 2.0 * eta / (6.0 * size + 1.0)
  for i in range(3):
    p = pred[i]
    df = scale * (ans(others, i) - p) / max(0.1, 4.0 * p * (1.0 - p))
    thr[i] -= df
    thr[i] *= decay
    for j in range(6 * size):
