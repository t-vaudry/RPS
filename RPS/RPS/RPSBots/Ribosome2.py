	...
	while dynein>10:
		...
		for i in range(dynein):
			...
			if j+tryptophan<length and not tryptophan == -1:
				protein[0]+={'R':isomer,'P':0,'S':-isomer}[RNA_strand[tryptophan+j]]
				protein[1]+={'R':-isomer,'P':isomer,'S':0}[RNA_strand[tryptophan+j]]
				protein[2]+={'R':0,'P':-isomer,'S':isomer}[RNA_strand[tryptophan+j]]
		j+=1
		dynein = peptide.count(peptide[length-j-1:length-1])
	if protein!=[0,0,0]:
