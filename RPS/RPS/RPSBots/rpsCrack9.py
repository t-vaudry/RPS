...
class HistoryNode(object):
    ...
    def new_move(self, input):
        ...
        if len(input) > 2:
            ...
            self.distribution[last_move] += 1 * 0.9975 ** (1000-counter)
...
if input == '':
    ...
    if pred[prediction]+input in win:
        state[0] += 1 * 0.9975 ** (1000-counter)
    elif pred[prediction]+input in lost:
        state[1] += 1 * 0.9975 ** (1000-counter)

    prediction = history_tree.predict()        

    #very simple switching / adapting strategie        
    if counter < 25 or random.random() < 0.5 or state[0] < state[1]:
        output = random.choice(["R","P","S"])
    else:
