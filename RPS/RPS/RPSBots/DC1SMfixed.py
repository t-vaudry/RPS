...
class RPSMDL:
    def __init__(self, order, adapt, sense):
        ...
        self.match = 0
        if adapt == 0:
            self.mdl = [[0,0,0] for a in range(self.mod)]
        else:
            self.mdl = [[IMID,IMID,IMID] for a in range(self.mod)]
    def predict(self):
...
class RPSPREDICTOR:
    def __init__(self, maxord):
        self.models = [[[RPSMDL(0, 0, 0) for a in range(2)] for b in range(2)] for c in range(maxord)]
        self.weights = [[[IMID,IMID] for a in range(2)] for b in range(maxord)]
        self.last = [[[0,0] for a in range(2)] for b in range(maxord)]
        self.maxord = maxord
