###############################################################################
#                                                                             #
# This is Chainmail375, an RPS program written by PotentProton.               #
#                                                                             #
...
# guarantee a 37.5% win rate against even the toughest opponents.             #
#                                                                             #
...
# strategy has a 37.5% chance of a win. This type of strategy is superior to  #
...
# chosen to guarantee 37.5% success, but this comes at a cost. There is       #
...
if input == "":
	...
	# Switch to a random strategy if we risk not being able to guarantee a
	# 37.5% chance of a win.
	if score <= -7:
		randstrat = 1
	if now > 169 and score <= -6:
		randstrat = 1
	if now > 376 and score <= -5:
		randstrat = 1
	if now > 553 and score <= -4:
		randstrat = 1
	if now > 701 and score <= -3:
		randstrat = 1
	if now > 819 and score <= -2:
		randstrat = 1
	if now > 907 and score <= -1:
		randstrat = 1
	if now > 967 and score <= 0:
		randstrat = 1
	if now > 996 and score <= 1:
		randstrat = 1
