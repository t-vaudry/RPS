...
class RPSPatternMatcher:
  ...
  def next_throw(self):
    ...
    total_scores = [total_counts[(i+2)%3] - total_counts[(i+1)%3] for i in range(3)]
    dampen = sum(abs(i) for i in total_scores)
    total_scores = [i + (dampen/2) for i in total_scores]
