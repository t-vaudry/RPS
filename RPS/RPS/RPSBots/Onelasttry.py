...
def getMove(sideFreq, sideHist, lookback, pairs):
	...
	moveOptions[RPSmaps[freqPredict    ]] += 1
	moveOptions[RPSmaps[lookbackPredict]] += 7
	
	pairPredict = getPairPredict(getBestRPS(moveOptions), pairs)
	
	moveOptions[RPSmaps[pairPredict]] += 7
	if(moveOptions[R] != 8 or moveOptions[P] != 8 or moveOptions[S] != 8):
		moveOptions = [0,0,0]
		moveOptions[RPSmaps[hennyPredict]] += 5
	return moveOptions
if(input == ""):       
