# Should fix a bug in switching2
...
if input == "":
  ...
  predicted[0] = anti_expected[random.choice(opp)]
  ...
  for length in range(min(4, len(dna)-1), 0, -1):
    ...
    if idx != -1:
      ...
      predicted[1] = anti_expected[dna_decode[answered][0]]
      break
  ...
  for length in range(min(4, len(dna)-1), 0, -1):
    ...
    if idx != -1:
      ...
      predicted[2] = anti_expected[anti_expected[myexpected]] 
      break
