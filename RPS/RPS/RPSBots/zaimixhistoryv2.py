# adds on pattern matching on both op and my hands
...
if not input:
            ...
            for i in xrange(self.bot_num):
                ...
                if payoff[LOSE] > payoff[WIN] and self.dropswitch:
                    ...
                    if curr_score is None:
                        ...
                        mix_scores(curr_score, payoff, self.decay_function(len(op_hands)))
    ...
    class HistoryMatch(object):
        def __init__(self, hands1, hands2):
            self.hands1 = hands1
            self.hands2 = hands2
        ...
        def update(self):
            hands_len = len(self.hands1)
            for order in xrange(min(ORDER, hands_len - 1)):
                index = tuple(self.hands1[-order-2:-1]) + tuple(self.hands2[-order-2:-1])
                self.counts[index] = (hands_len - 1)
            self.matched_index = None
            for order in xrange(min(ORDER, hands_len) - 1, -1, -1):
                index = tuple(self.hands1[-order-1:]) + tuple(self.hands2[-order-1:])
    ...
    class HistoryMatchBot(object):
        ...
        def update(self):
            ...
            if matched_index is None:
                self.next_hand = None
                return
            nexth = self.history_matcher.hands1[matched_index]
    ...
    def stablizing_decay(n, sqrt=math.sqrt):
        # return (n / 1000.0) ** (1.0/3.0) / 2.0 + 0.47
        return 0.9
