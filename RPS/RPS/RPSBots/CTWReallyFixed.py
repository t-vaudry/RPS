if input == "":
    ...
    if dqs is not None:
        ...
        for x in xrange(3):
            if x == beat[i]:
                ...
                s = 0
            dscoreplus += dqsplus[x] * s
            dscoreminus += dqsminus[x] * s
        if dscoreplus >= 0:
            new_sign_plus = 1
        else:
            new_sign_plus = -1
        if dscoreminus >= 0:
            ...
            new_sign_minus = -1
        if new_sign_plus == signplus:
            learning_rate *= 1.05
        else:
            learning_rate *= 0.95
        if new_sign_minus == signminus:
            learning_rate *= 1.05
        else:
            learning_rate *= 0.95
        if learning_rate <= 0:
        ...
        if learning_rate >= 0.5:
            learning_rate = 0.5
        signplus = new_sign_plus
        signminus = new_sign_minus
        betaplus += learning_rate * dscoreplus
        betaminus += learning_rate * dscoreminus
        if betaplus >= 10:
            betaplus = 10
        if betaplus <= 0:
            betaplus = 0
    ...
    dqsplus = [0 for _ in xrange(3)]
    dqsminus = [0 for _ in xrange(3)]
    for x in xrange(3):
        for y in xrange(3):
            if x == beat[y]:
                s = 1
            ...
            e1 = exp(betaminus * s)
            qs[x] += ps[y] * (e0 + e1)
            dqsplus[x] += ps[y] * e0 * betaplus
            dqsminus[x] += ps[y] * e1 * betaminus
    t = sum(qs)
    p = 0
