...
class RPSMDL:
    def __init__(self, order, decay, init):
        ...
        self.ctx = 0
        self.rate = decay
        self.mdl = [[float(init), float(init), float(init)] for a in range(self.mod)]
    def predict(self):
...
class RPSPREDICTOR:
    def __init__(self, maxord, adapt):
        self.models = [[RPSMDL(0, 0, 0), RPSSMDL(0, 0, 0)] for a in range(maxord + 1)]
        self.weights = [[IMID, IMID] for a in range(maxord + 1)]
        self.last = [[0, 0] for a in range(maxord + 1)]
        self.maxord = maxord
        self.rate = adapt
        self.prev = 0
