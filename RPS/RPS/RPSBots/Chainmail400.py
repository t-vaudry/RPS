###############################################################################
#                                                                             #
# This is Chainmail400, an RPS program written by PotentProton.               #
#                                                                             #
...
# guarantee a 40% win rate against even the toughest opponents.               #
#                                                                             #
...
# strategy has a 40% chance of a win. This type of strategy is superior to    #
...
# chosen to guarantee 40% success, but this comes at a cost. There is         #
...
if input == "":
	...
	# Switch to a random strategy if we risk not being able to guarantee a 40%
	# chance of a win.
	if score <= -6:
		randstrat = 1
	if now > 12 and score <= -5:
		randstrat = 1
	if now > 293 and score <= -4:
		randstrat = 1
	if now > 526 and score <= -3:
		randstrat = 1
	if now > 713 and score <= -2:
		randstrat = 1
	if now > 854 and score <= -1:
		randstrat = 1
	if now > 947 and score <= 0:
		randstrat = 1
	if now > 994 and score <= 1:
		randstrat = 1
