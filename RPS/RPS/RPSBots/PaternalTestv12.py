...
if not input:
    ...
    opp_history = ""
    hind_sight = 6
    strats = ["pattern", "Lizard", "Spock", "Shotgun", "Jellyfish", "Same", "Ditto", "Dynamite", "Zombie"]
    strat_stat = {"pattern": 0, "Lizard": 0, "Spock": 0, "Shotgun": 0, "Jellyfish": 0, "Same": 0, "Ditto": 0, "Dynamite": 0, "Zombie": 0}
    strat_predict = {"pattern": "", "Lizard": "", "Spock": "", "Shotgun": "", "Jellyfish": "", "Same": "", "Ditto": "", "Dynamite": "", "Zombie": ""}
        #Possible strategies:
    ...
    if current_strat == "pattern":
        output = beats[strat_predict["pattern"]]
    elif current_strat == "Lizard":
        output = beats[strat_predict["Lizard"]]
    elif current_strat == "Spock":
        output = beats[strat_predict["Spock"]]
    elif current_strat == "Shotgun":
        output = beats[strat_predict["Shotgun"]]
    elif current_strat == "Jellyfish":
        output = beats[strat_predict["Jellyfish"]]
    elif current_strat == "Same":
        output = beats[strat_predict["Same"]]
    elif current_strat == "Ditto":
        output = beats[strat_predict["Ditto"]]
    elif current_strat == "Dynamite":
        output = beats[strat_predict["Dynamite"]]
    else:
        output = beats[strat_predict["Zombie"]]