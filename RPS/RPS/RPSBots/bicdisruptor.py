...
class RPSPatternMatcher:
  ...
  def register_throw(self,our_throw, opponent_throw):
    if self.history:
      ...
      self.prediction_history.append((self.prediction - opponent_throw + 4) % 3 - 1)
  ...
  def next_throw(self):
    ...
    total_scores = [i for i in total_scores]
...
# pretty sure this is borderline malicious...
try: our_max
except NameError:
  our_max = __builtins__.max
  our_min = __builtins__.min
  our_len = __builtins__.len
  our_range = __builtins__.range
  types = ["R", "P", "S"]
