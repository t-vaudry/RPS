...
if input == "":
	output = choice[random.randint(0,2)]
	matchCount = 0
	inStore = [[],[]]
elif input != "":
	...
	if matchCount < stopRand:
		...
		if   (sCH == rCH) and (sCH == pCH):
			output = choice[random.randint(0,2)]
		elif (sCH == rCH) and (sCH >  pCH):
			output = choice[(random.randint(0,1) * 2)]
		elif (sCH == pCH) and (sCH >  rCH):
			output = choice[random.randint(1,2)]
		elif (rCH == pCH) and (rCH >  sCH):
			output = choice[random.randint(0,1)]
		elif (rCH >  pCH) and (rCH >  sCH):
			output = choice[1]
