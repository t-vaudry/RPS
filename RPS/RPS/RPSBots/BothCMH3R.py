...
if input == "":
    ...
    nval = {"R":0,"P":1,"S":2}
    cval = ["R","P","S"]
    mdl = [CtxModel(i % 7, 0.93) if i < 14 else CtxModel((i % 7) * 2, 0.93) for i in range(0, 21)]
    msse = ModelSwitch(64, 0.93)
    msse.setoffset(63, -0.1)
    lval = 0
else:
    val = nval[input]
    for i in range(0, 7):
        ...
        mdl[i+14].update(val)
        mdl[i+7].update(lval)
        mdl[i+14].update(lval)
    msse.update(val)
pv = 0
for i in range(0, 21):
    pv = mdl[i].predict()
    msse.setvalue(i, pv)
    msse.setvalue(i + 21, winner[pv])
    msse.setvalue(i + 42, winner[winner[pv]])
msse.setvalue(63, random.randint(0, 2))
