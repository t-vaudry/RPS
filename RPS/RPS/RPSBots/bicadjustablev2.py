...
class RPSPatternMatcher:
  ...
  def next_throw(self):
    ...
    if self.prediction_history and len(self.prediction_history) % 30 == 0:
      prob_score = sum([p * c for c,p in zip(self.confidence_history, self.prediction_history) if c > .8][-30:])
      self.randomness = max(0,min(1, 1 - max(prob_score, 0)/5))
