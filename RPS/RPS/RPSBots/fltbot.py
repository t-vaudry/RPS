# modified from dllu1 (changed constants), see also www.dllu.net/rps
import random
numPre = 30
numMeta = 6
if not input:
    ...
    for i in range(numPre):
        ...
        bpp = beat[pp]
        bbpp = beat[bpp]
        pScore[0][i]=0.95*pScore[0][i]+((input==pp)-(input==bbpp))*3
        pScore[1][i]=0.95*pScore[1][i]+((output==pp)-(output==bbpp))*3
        pScore[2][i]=0.85*pScore[2][i]+(input==pp)*3.3-(input==bpp)*1.2-(input==bbpp)*2.3
        pScore[3][i]=0.85*pScore[3][i]+(output==pp)*3.3-(output==bpp)*1.2-(output==bbpp)*2.3
