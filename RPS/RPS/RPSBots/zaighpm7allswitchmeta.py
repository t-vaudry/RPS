...
# choose the meta strategy with the best score
# a little bit of decay goes a long way
# this bot is deterministic
...
if not input:
	...
	meta_scores = [0.0] * META_STRATEGIES
...
# --------------------- turn -----------------------------
	...
	if hands_played > 1:
		...
		for i in xrange(META_STRATEGIES):
			meta_scores[i] += topoints[scoreround[next_hands_meta[i]][op_last_hand]]
	...
	max_meta_score = max(meta_scores)
	max_meta_list = [i for i in xrange(META_STRATEGIES) if meta_scores[i] == max_meta_score]
